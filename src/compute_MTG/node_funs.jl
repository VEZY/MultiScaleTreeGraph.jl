...
function _subtree_has_node_id_conflict(node::Node, store::MTGAttributeStore)
    ...
    while !isempty(stack)
        current = pop!(stack)
        nid = node_id(current)
        nid in seen_ids && return true
        push!(seen_ids, nid)
        if nid <= length(store.node_bucket) && store.node_bucket[nid] != 0
            return true
        end
        current_children = children(current)
        @inbounds for child_index in reverse(eachindex(current_children))
            push!(stack, current_children[child_index])
        end
    end
    return false
end
...
function _merge_columnar_subtree_into_store!(node::Node, target_store::MTGAttributeStore)
    ...
    empty!(attrs.staged)
    if _maybe_traversal_cache(node) !== nothing
        source_store === nothing || source_store === target_store ||
            _unregister_traversal_cache!(source_store, node)
        _register_traversal_cache!(target_store, node)
    end
...
function _try_merge_subtree_store!(child::Node, parent_store::MTGAttributeStore, child_store::MTGAttributeStore)
...
function _validate_columnar_attach!(
    p::Node,
    child::Node,
    reserved_ids::Union{Nothing,Set{Int}}=nothing,
)
    ...
    if p_store !== nothing && c_store !== nothing && p_store !== c_store
        reserved_ids === nothing && (reserved_ids = Set{Int}())
        subtree_ids = Set{Int}()
        stack = typeof(child)[child]
        while !isempty(stack)
            current = pop!(stack)
            nid = node_id(current)
            if nid in subtree_ids ||
               (reserved_ids !== nothing && nid in reserved_ids) ||
               (nid <= length(p_store.node_bucket) && p_store.node_bucket[nid] != 0)
                throw(ArgumentError(
                    "cannot attach subtree rooted at node $(node_id(child)): node id $nid is duplicated or already present in the destination attribute store",
                ))
            end
            push!(subtree_ids, nid)
            current_children = children(current)
            @inbounds for child_index in reverse(eachindex(current_children))
                push!(stack, current_children[child_index])
            end
        end
        union!(reserved_ids, subtree_ids)
    end
    return reserved_ids
end

@inline function _maybe_recolumnarize_after_attach!(p::Node, child::Node)
    p_store = _columnar_store_or_nothing(p)
    c_store = _columnar_store_or_nothing(child)
    if p_store !== nothing && c_store !== nothing && p_store !== c_store
        _try_merge_subtree_store!(child, p_store, c_store) || error(
            "failed to merge the attached subtree into its destination attribute store",
        )
    end
    return nothing
end
...
function addchild!(p::Node{N,A}, child::Node; force=false) where {N<:AbstractNodeMTG,A}
    if parent(child) !== nothing && parent(child) !== p && force == false
