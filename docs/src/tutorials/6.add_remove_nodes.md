...
The first argument to [`Node`](@ref) is its MTG description: link with parent, scale, index, and symbol. It is given as a [`MutableNodeMTG`](@ref) (or a [`NodeMTG`](@ref)). The second argument contains node attributes.
...
Adding nodes step by step is easy, but sometimes we want to insert nodes in-between other nodes. We can still use `Node` to do so, but it becomes a bit cumbersome because you'll have to handle manually the changes in parents, children and siblings.
...
!!! note
    [`delete_nodes!`](@ref) always returns the root node of the resulting MTG.
    If the old root was deleted and had one child, that child becomes the new root.
    If the old root had several children, the function returns an error.
    In practice, re-assign the result to keep the correct root reference after deletions.
