...
[`transform!`](@ref) uses [`traverse!`](@ref) under the hood to apply a function call to each node of an MTG. [`traverse!`](@ref) is just a little bit less easy to use as only Form 4 exists. We can obtain the exact same results as the last example using the same call with [`traverse!`](@ref). Let's change the `Leaf` symbol into `L`:
